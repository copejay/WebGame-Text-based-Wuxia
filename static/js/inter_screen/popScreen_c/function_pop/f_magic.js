...
import { AU_Button}  from "/static/js/inter_screen/popScreen_c/function_pop/tool_bag/magic_button.js";
import {magicPopC} from "/static/js/inter_screen/popScreen_c/function_pop/popC/magicPopC.js"
import event_bus from "/static/js/event_bus.js";


export class f_magic{
    constructor(site,size,data,scene){
        ...
        this.magicPopC=new magicPopC(this.site,this.size,this.data,this.scene);

    }

    upDescribe(data){

        event_bus.off("_Pop_f_magic_click");

        event_bus.on("_Pop_f_magic_click",(data)=>{
            console.log(data);
            this.magicPopC.run(data);
        })
        ...
        let text=this.scene.add.text(this.w*0.05,this.h*0.05,"【武功】");
        text.setOrigin(0,0);
        text.setFontSize(text_h);
        text.setColor("black");
        this.up_container.add(text);

        this.out_button=new AU_Button(this.w*0.9,this.h*0.07,this.w*0.12,this.h*0.08,"X","f_magic",this.scene);
        ...
        fix_list.forEach((data,num)=>{
            ...
            const newButton=new AU_Button(button_w*0.75+column*this.w*0.45,button_h*0.5+button_h*1.3*level,button_w,button_h,[data[0],data[1],data[2]],"f_magic",this.scene);
