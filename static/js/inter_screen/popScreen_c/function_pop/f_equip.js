...
import { AU_Button}  from "/static/js/inter_screen/popScreen_c/function_pop/tool_bag/equip_button.js";
import {equipPopC} from "/static/js/inter_screen/popScreen_c/function_pop/popC/equipPopC.js"
import event_bus from "/static/js/event_bus.js";



export class f_equip{
    constructor(site,size,data,scene){
        ...
        this.equipButton_list=[];

        this.equipPopC=new equipPopC(this.site,this.size,['null','null'],this.scene);

    }


    //_下划线表示内部通信，这里意思是，功能弹窗的具体按钮传出的点击信息
    //接收到点击之后，唤起具体操作弹窗
    create(data=null){
        event_bus.off("_Pop_f_equip_click");

        event_bus.on("_Pop_f_equip_click",(data)=>{
            console.log(data);
            //具体操作弹窗运行
            this.equipPopC.run(data);
        })
        ...
        let text=this.scene.add.text(this.w*0.05,this.h*0.05,"【装备栏】");
        text.setOrigin(0,0);
        ...
        this.out_button=new AU_Button(this.w*0.9,this.h*0.07,this.w*0.12,this.h*0.08,"X","","f_equip",this.scene);
        ...
        fix_list.forEach((data,num)=>{
            ...
            if(num%3==0){
                level+=1;
                row=0;
                const newButton=new AU_Button(button_w*0.6+row*this.w*0.32,button_h*1.2*(0.5+level),button_w,button_h,data[0],data[1][0],"f_equip",this.scene);
                this.equipButton_list.push(newButton);
        ...
        // this.equipButton_list.forEach((data)=>{
